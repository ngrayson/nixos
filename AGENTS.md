...
- `os-rebuild` and `hearth-deploy` use the **active checkout** and must tell the user which branch/lane they are on. They do not pick a branch.
...
- **Hearth** is built on the builder (usually Tawa) and activated with `hearth-deploy` (`scripts/hearth-deploy.sh`) from that checkout. Do not routine-switch Hearth on-box. Exception: the first on-box Hearth switch after `hosts/Hearth/remote-access.nix` trust/sudo changes.
