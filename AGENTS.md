...
- Before asking the user to activate, run `os-rebuild build` (this host) or `hearth-deploy build` (Hearth) and only ask if it succeeds. Then give them `os-rebuild switch` / `hearth-deploy switch` plus a check. In a codespace, `nix flake check` is the dry-run (`CURSOR_SETUP.md`, `.devcontainer/conveyor/setup.sh`); never `nixos-rebuild` / `os-rebuild switch` there.
...
- Hardware / kernel / disk / Hearth headless flip: user-run `dry-activate` then `boot`, then reboot. `test` is not a rollback.
