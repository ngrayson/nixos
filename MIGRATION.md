...
6. In **[`configuration.nix`](./configuration.nix)**, the shared defaults stay as-is. Edit **[`hostname.nix`](./hostname.nix)** (or replace the `./hostname.nix` import with another file) for the new host: **`networking.hostName`**, **`<nixos-hardware/...>`** (or remove that import if no profile fits), any extra **LUKS** entries, and **`boot.kernelParams`** (e.g. drop **`amd_pstate=active`** on Intel).
...
`configuration.nix` also imports [`./hostname.nix`](./hostname.nix) for these Theseus settings. A **gitignored** `local.nix` is still an option for secrets only you do not want in git, but host identity and disk UUIDs are expected to be edited or swapped per machine in `hostname.nix` / `hardware-configuration.nix` as you document in the log above.
