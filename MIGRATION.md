...
- **2026-04-26 — Stylix (theme-first):** **[`home/stylix.nix`](./home/stylix.nix)** imports **`(import «stylix-src»).homeModules.stylix`** with **`builtins.fetchTarball`** on **`release-25.11`** ([Stylix installation](https://nix-community.github.io/stylix/installation.html); tarball instead of **`pkgs.fetchFromGitHub`** in the same module avoids HM import/`pkgs` recursion). **`stylix.autoEnable = false`**; **`fontconfig`**, **`gtk`**, **`hyprland`** + **Hyprpaper**. **`programs.dconf.enable`** in [`common/system.nix`](./common/system.nix). **`tokyonight-gtk-theme`** removed from `environment.systemPackages`. **Qt:** **`stylix.targets.qt.enable = false`** — **Kvantum** + [`home/lib/host-xdg.nix`](./home/lib/host-xdg.nix). **Deferred:** Quickshell bar/lock, Albert, dunst / PipeWire UI (optional **`stylix.targets.dunst`** after HM-managed dunst).
