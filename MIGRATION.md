...
The **Framework / Theseus** phased roadmap (LOCKED, audit, session checklist, rice, etc.) is **archived** under **`documentation/nixos-framework-setup/archive/`** — reference only on **Tawa**; it does not change `nixos-rebuild`. This directory also keeps **[`documentation/nixos-framework-setup/os-rebuild.sh`](./documentation/nixos-framework-setup/os-rebuild.sh)** (rebuild helper). If you still maintain a copy under a Stellarium checkout (`projects/nixos-framework-setup/`), **sync manually** when you want them to match.

**Home Manager layout:** Root [`home.nix`](./home.nix) is only `imports = [ ./home ];`. **[`home/default.nix`](./home/default.nix)** pulls in modules such as [`home/session.nix`](./home/session.nix), [`home/programs/`](./home/programs/), [`home/wayland/hyprland.nix`](./home/wayland/hyprland.nix), [`home/services/hypridle.nix`](./home/services/hypridle.nix), [`home/xdg/`](./home/xdg/), [`home/hypr/scripts.nix`](./home/hypr/scripts.nix), and [`home/lib/`](./home/lib/). Optional follow-ups (rice, extra `xdg.*`, Stylix, etc.) are your choice; historical Framework checklists live in **`documentation/nixos-framework-setup/archive/`** (2026-04-05 snapshot). This repo’s **live** layout is described here and in the migration log.
...
| `~/.config/kitty/kitty.conf` + `izar.conf` | HM `xdg.configFile` in [`home/xdg/config.nix`](./home/xdg/config.nix) from [`kitty/`](./kitty/) in this repo; **`kitty` package in `environment.systemPackages`** (Plasma shortcuts need system `PATH`) | **Done (step 3).** |
...
| GTK / Hyprland colors / wallpaper / session fonts | **[`home/stylix.nix`](./home/stylix.nix)** (Home Manager only; [`home/default.nix`](./home/default.nix)) | Stylix is a pinned **flake input** (`flake.nix` → `inputs.stylix`, `release-26.05`). Palette and wallpaper per host live in [`home/theme/hosts.nix`](./home/theme/hosts.nix) (not a fetchTarball). **`stylix.targets.kitty.enable = false`** — Kitty stays [`home/xdg/config.nix`](./home/xdg/config.nix). |

### Home Manager migration log

Iterative: **one** logical change per rebuild; **verify** before the next (optional context: archived Framework roadmap in `documentation/nixos-framework-setup/archive/`).
...
- **2026-04-17 — Step 3 (Kitty):** Config from [`./kitty/kitty.conf`](./kitty/kitty.conf) + [`./kitty/izar.conf`](./kitty/izar.conf) (Izar / chromamancer; `lilac-ash.conf` archived in repo) via `xdg.configFile` in **[`home/xdg/config.nix`](./home/xdg/config.nix)** (not `programs.kitty` — avoids a second generated `kitty.conf`). **`kitty` is in `environment.systemPackages`** so Plasma keyboard shortcuts see it on the system `PATH` (user-only `home.packages` was not enough for Alt+Enter–style launchers). `termconfig` alias edits the **nixos repo** `kitty/` files. Rebuild: `sudo nixos-rebuild switch`. **Verify:** new Kitty. If the first `switch` failed with *would be clobbered*, those `xdg` entries use `force = true` to replace existing files (back up first if you need the old `kitty.conf`).
...
- **2026-04-26 — `hosts/Theseus/` (Framework laptop):** Entrypoint mirrors **Tawa**. *(Historical: this log called `hardware-configuration.nix` a stub. Live file now has real UUIDs — see [`hosts/Theseus/README.md`](./hosts/Theseus/README.md).)* **`kvantum/Theseus/`** copies the Tawa theme layout until you customize on-device.
...
- **2026-04-26 — Stylix (theme-first):** *(Historical snapshot.)* At the time, [`home/stylix.nix`](./home/stylix.nix) imported Stylix via **`builtins.fetchTarball`** on **`release-25.11`**. **Live pin is the flake input** `stylix` / `release-26.05` in [`flake.nix`](./flake.nix). Host palettes and wallpapers are [`home/theme/hosts.nix`](./home/theme/hosts.nix). Do not re-add `fetchTarball` or a 25.11 pin.
