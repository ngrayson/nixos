...
- `qs-quickshell-ipc call switcher next|prev|commit|dismiss|toggle` — alt-tab picker. `next`/`prev` open it or step the highlight, `commit` focuses the highlighted window and closes, `dismiss` closes without changing focus, `toggle` is the mouse-browse path. Note `commit`/`dismiss` have **no key bound to them**: committing on Alt release was reverted because Hyprland 0.55.4 does not deliver a release bind for a modifier that took part in another bind (and Alt+Tab always does). The overlay holds **exclusive keyboard focus**, so anything that opens it over IPC must close it again (`dismiss`) before handing the session back.
